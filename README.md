...
| M0 專案骨架 | ✅ |
| M1 TDX 資料模型與解析 | ✅ |
| M2 靜態索引 + 模糊搜尋 + 候選集合匹配 | ✅ 527 項離線驗收測試全過 |
...
| **M11 AI 聊天**（OpenAI 相容 API ＋ Semantic Kernel） | ✅ @ 提及／回覆觸發、時間＋LLM 切段、每週全域 token 上限、工具呼叫真的能訂閱公車、**可以被馴服（每個伺服器自己的提示詞，`/rest` 可重設）**、**能幫使用者按面板按鈕**、**會偷聽後幾句（多人聊天時不會被忽略）**（實測真實 API 通過） |
| M5 / M6 | ⬜ |
...
```powershell
# 527 項驗收測試：搜尋、群組、匹配、通知判定、簡繁折疊、縮寫、輪詢成本、儲存後端與 DI 註冊、.env 路徑、合併與復原、結束追蹤、AI 聊天切段與每週額度、LLM 工具訂閱、可馴服的提示詞、認人與主人授權、面板與 AI 共用站牌解析、路線號碼與轉乘查詢、模型思考開關、偷聽模式（含「沒窗口就不花錢問」、三段判斷、@ 別人不會退出、閒聊留下來當上下文）、對話記憶容量上限與學到的規則上限（每段／每頻道／頻道數／TTL／規則條數）、SQLite 持久化
