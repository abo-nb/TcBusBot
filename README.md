...
| M0 專案骨架 | ✅ |
| M1 TDX 資料模型與解析 | ✅ |
| M2 靜態索引 + 模糊搜尋 + 候選集合匹配 | ✅ 494 項離線驗收測試全過 |
...
| **M11 AI 聊天**（OpenAI 相容 API ＋ Semantic Kernel） | ✅ @ 提及／回覆觸發、時間＋LLM 切段、每週全域 token 上限、工具呼叫真的能訂閱公車、**可以被馴服（每個伺服器自己的提示詞，`/rest` 可重設）**、**能幫使用者按面板按鈕**、**會偷聽後幾句**（實測真實 API 通過） |
| M5 / M6 | ⬜ |
...
/ai status     ← AI 聊天狀態：模型、這一週用掉多少 token、這個頻道的記憶與偷聽狀態
...
模型回完話以後，同一個頻道接下來幾句**會繼續聽**，這樣不用每句都 @ 它：
...
小華：那 304 呢？                    ← 沒 @ 它，但在跟它說話
Bot ：304 也有到，停的站比較多        ← ✅ 接話
      🔧 我實際做了：search_routes（304）

小華：你要不要一起去吃火鍋？          ← 判斷「不是在跟我說話」
      （不出聲、停止偷聽，回到原本的「等 @」模式）

小華：那我先走囉                      ← 沒 @ 它 → 完全不理（連判斷都不做）
```

- **判斷由 LLM 做**（一次很便宜的短判斷，輸出上限 8 tokens）：
  不確定時一律回「不是在跟我說話」—— 寧可少回一句，也不要插話
- **明顯在叫別人就完全不問**：訊息裡 @ 了其他人 → 直接停止偷聽（**不花任何 token**，也不靠模型判斷）
- **不是對它說 → 停止偷聽**：回到「只能 @ 或回覆它」的模式，不會一直被動回話
- **兩個上限同時管**：`LLM_EAVESDROP_MESSAGES`（幾則）與 `LLM_EAVESDROP_SECONDS`（幾秒），
  先到的那個就停；`/ai status` 看得到還剩幾則／幾秒
...
| `LLM_EAVESDROP` | `true` | 回完話後**再偷聽幾句**（不是在跟它說話就停止，回到等 @） |
| `LLM_EAVESDROP_MESSAGES` | `3` | 偷聽最多幾則訊息（`0` = 關閉） |
| `LLM_EAVESDROP_SECONDS` | `120` | 偷聽最多幾秒（時間或則數先到就停） |
...
```powershell
# 494 項驗收測試：搜尋、群組、匹配、通知判定、簡繁折疊、縮寫、輪詢成本、儲存後端與 DI 註冊、.env 路徑、合併與復原、結束追蹤、AI 聊天切段與每週額度、LLM 工具訂閱、可馴服的提示詞、認人與主人授權、面板與 AI 共用站牌解析、路線號碼與轉乘查詢、模型思考開關、偷聽模式（含「沒窗口就不花錢問」與「不是在跟我說話就停止」）、SQLite 持久化
