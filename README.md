...
| M0 專案骨架 | ✅ |
| M1 TDX 資料模型與解析 | ✅ |
| M2 靜態索引 + 模糊搜尋 + 候選集合匹配 | ✅ 678 項離線驗收測試全過 |
...
| 工具 | 做什麼 | 會改資料嗎 |
| --- | --- | --- |
| `search_stops` | 查站牌的正確名稱＋**有哪幾條路線經過**（**跨城市找**，並標出是哪個城市；可用 `city` 指名城市） | 不會 |
| `search_routes` | 用**路線號碼**查（「300 到哪裡」「304 幾站」） | 不會 |
| `find_routes` | 查「A 到 B」可以搭哪些車；**沒有直達時給「轉一次」的走法**（跨城市、可用 `city` 指名城市） | 不會 |
| `list_subscriptions` | 列出已經訂了什麼 | 不會 |
| `next_arrivals` | 查還要幾分鐘到站（**所有訂閱組都會查**） | 不會 |
| `subscribe_bus` | **真的建立訂閱**（與 `/bus panel` 同一套站序匹配邏輯；可用 `city` 指名城市） | ✅ 會 |
...
BUS_CITY=Tainan               # 只跑臺南
BUS_CITY=Taichung,Tainan      # ★ 臺中 ＋ 臺南「同時」跑（逗號、頓號、空白都可以分隔）
BUS_CITY=台中、台南            # 中文也可以
```
...
- 換城市之後 `/bus status`、`/health`（`city` 欄位）、啟動畫面的資料來源都會顯示實際城市，
  面板標題也會變成「🚌 臺南公車訂閱」
- **模型一定會被告知服務哪個城市**：系統提示固定加一段「【服務範圍】你查到的公車資料都是**臺南**的…不要回答其他縣市」。這一段**不受自訂提示詞影響** —— 之前只靠預設人格的 `{city}` 佔位，主機一旦自訂 `LLM_SYSTEM_PROMPT`，模型就完全不知道自己服務哪個縣市（跑臺南還是會聊台中公車）。`/ai status` 也看得到「服務城市」
- ⚠️ 內建最小資料集（沒有 TDX 金鑰時的離線資料）**只有臺中走廊**：
  非臺中時必須有 TDX 金鑰或已抓好快取，不然 Bot 會直接說出這件事，而不是給你錯的站牌
- 實測：`BUS_CITY=Tainan` 從 TDX 抓到 **11,329 個站牌**（臺中為 14,036）；兩個一起跑時是
  **兩份各自獨立**的資料（臺中 14,036／755 筆、臺南 11,329／323 筆），不是合併成一份 ——
  這樣才不會有「忘了過濾城市」的路徑

#### 讓「要訂公車的人」自己選城市（`/bus city`）

```
主機端（決定「有哪些城市可以選」）：
BUS_CITY=Taichung,Tainan      ← 載入臺中與臺南兩份資料

使用者端（決定「我要看哪一個」）：
/bus city                     ← 看目前選哪個、有哪些可以選
/bus city 城市:臺南            ← 換成臺南（只影響你自己）
```

- **每個城市各一份資料**（不是合併再過濾）：合併的做法只要漏掉任何一條查詢路徑，
  就會吐出別的城市站牌而且不會報錯；一個城市一份索引，「選哪一份」由資料目錄決定，
  **不可能看錯城市**
- **即時到站會按城市分批查**：ETA 的網址裡有城市（`/City/{City}`），
  不按城市分批的話，臺南的站牌會被拿去問臺中端點 → 一直沒有到站時間
  （`BusDataService.GroupByCity` 負責這件事，`--dryrun` 會驗）
- 每個城市各自一份快取（`Stop.Taichung.json`／`Stop.Tainan.json`），互不干擾
- **預設＝`BUS_CITY` 的第一個城市**（通常是臺中）：沒選過的人行為跟以前完全一樣
- **按使用者**選，不是按伺服器：同一個伺服器裡有人通勤看臺中、有人回老家看臺南都行
- **選過的選擇會存起來**（跟訂閱同一條後端鏈），重啟後還在
- **已經訂好的訂閱不會因為換城市失效**：即時到站是依「站牌自己的城市」去查的
- AI 也會跟著你選的城市（`/ai status` 看得到「你要查的城市」）
- **AI 的工具會自己跨城市找**：你問「臺南車站到安平要搭幾號」時，即使你的城市還是預設的臺中，
  它也查得到（結果會標「（城市：臺南）」）。判斷順序是**強相符的城市優先**
  —— 因為在臺中搜「臺南車站」也會撈到長得很像的「日南車站」，
  照「第一個有回應的城市」走就會回你**臺中**的答案，而且看起來像真的
- **AI 也可以指名城市**：你說「我要查臺南的」時，它會把城市一起送進工具（`city` 參數），
  就只查那一個城市；指名沒載入的城市時，它會回你「我能查的城市：臺中、臺南」
- **已經訂好的訂閱不會因為換城市失效**：即時到站是依「站牌自己的城市」去查的
- 代價：靜態資料量變兩倍（記憶體）、啟動第一次要多抓一份（3 個 TDX 請求）
...
| `BUS_CITY` | `Taichung` | **要載入哪些城市的公車資料**（別名 `TCBUS_CITY`／`TDX_CITY`）。可多個（`Taichung,Tainan`）；使用者再用 `/bus city` 選自己要查哪一個 |
...
```powershell
# 678 項驗收測試：搜尋、群組、匹配、通知判定、簡繁折疊、縮寫、輪詢成本、儲存後端與 DI 註冊、.env 路徑、合併與復原、結束追蹤、AI 聊天切段與每週額度、LLM 工具訂閱、可馴服的提示詞、認人與主人授權、面板與 AI 共用站牌解析、路線號碼與轉乘查詢、模型思考開關、偷聽模式（含「沒窗口就不花錢問」、三段判斷、@ 別人不會退出、閒聊留下來當上下文、不會顯示假的「正在輸入」）、對話記憶容量上限與學到的規則上限（每段／每頻道／頻道數／TTL／規則條數、管理員 /ai pset／跨伺服器複製設定／按鈕授權／雙模型／多人排隊／多城市／看圖／使用者自選城市／LLM 工具跨城市（強相符優先）與模型指名城市）、SQLite 持久化
