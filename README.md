...
| M0 專案骨架 | ✅ |
| M1 TDX 資料模型與解析 | ✅ |
| M2 靜態索引 + 模糊搜尋 + 候選集合匹配 | ✅ 684 項離線驗收測試全過 |
...
- **管理員可以當場重置**：`/ai status` 底部有一顆「♻️ 重置本週額度」（只有
  `LLM_ADMIN_IDS` 名單上的人看得到）。按下去**先跳確認**（顯示目前用了多少），
  再按「✅ 確定重置」才會把這一週歸零 —— 不必改環境變數、也不必等週一。
  按鈕只認**開啟它的那個人**，而且按下時會再檢查一次身分；每次重置都會記進
  `/ai audit`（誰、什麼時候、把多少用量歸零）。額度用完時，管理員看到的訊息
  也會直接提示去哪裡按

> 上限是軟性的：最後一次呼叫可能稍微超過（最多一次回覆的量）。
> 重置**不會退錢**，只是重新計算這一週的上限。
...
| `/ai audit` | 看最近的主人操作紀錄（**只有主人看得到**） |
| `/ai status`（按鈕） | 管理員會多一顆「**♻️ 重置本週額度**」——把這一週的 token 用量歸零（按兩次才生效，會留紀錄） |
...
- **額度用完**：會回一句「這個星期的 AI 額度用完了」，`/ai status` 看得到剩下多少
  （管理員的訊息後面會多一句：去 `/ai status` 按「♻️ 重置本週額度」就能立刻恢復）
