...
| **M11 AI 聊天**（OpenAI 相容 API ＋ Semantic Kernel） | ✅ @ 提及／回覆觸發、時間＋LLM 切段、每週全域 token 上限、**工具呼叫真的能訂閱公車**（實測真實 API 通過） |
| M5 / M6 | ⬜ |
...
| 工具 | 做什麼 | 會改資料嗎 |
| --- | --- | --- |
| `search_stops` | 查站牌的正確名稱（模糊比對：台中/臺中、台中科大→國立臺中科技大學） | 不會 |
| `find_routes` | 查「A 到 B」可以搭哪些車 | 不會 |
| `list_subscriptions` | 列出已經訂了什麼 | 不會 |
| `next_arrivals` | 查還要幾分鐘到站（要 TDX 金鑰） | 不會 |
...
Bot ：訂好了 ✅ 共 2 條路線，提前 10 分鐘通知：
      • 300 → 在臺中車站(A月台)上車　• 304 → 在臺中車站(臺灣大道)上車
      🔧 我實際做了：search_stops（台中車站）、search_stops（靜宜大學）、
                     subscribe_bus（…→ 靜宜大學(專用道)，2 筆訂閱）

小明：靜宜大學到台中車站有哪些公車可以搭？先不要幫我訂閱
Bot ：（只用 find_routes 查詢，沒有建立訂閱）✅
```

- 模型一次可以連續呼叫多個工具（先查站牌、再訂閱），全部在**一次**呼叫裡完成
- **只能動「問的人」的資料**：工具參數裡沒有任何「幫誰訂」的欄位，模型不可能幫別人訂
- 模型**真的動了資料**時，回覆後面會附一行 `🔧 我實際做了：…` 讓你能核對（模型常常講得比做得多）
- 取消訂閱會掛「↩️ 復原」按鈕 —— 跟 `/bus end` 用同一個復原機制
- 站名不確定時會**先問你**，不會自己猜一個看起來像的（猜錯會訂到錯的路線）
