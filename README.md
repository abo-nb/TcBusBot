...
| M0 專案骨架 | ✅ |
| M1 TDX 資料模型與解析 | ✅ |
| M2 靜態索引 + 模糊搜尋 + 候選集合匹配 | ✅ 455 項離線驗收測試全過 |
...
| **M11 AI 聊天**（OpenAI 相容 API ＋ Semantic Kernel） | ✅ @ 提及／回覆觸發、時間＋LLM 切段、每週全域 token 上限、工具呼叫真的能訂閱公車、**可以被馴服（每個伺服器自己的提示詞，`/rest` 可重設）**（實測真實 API 通過） |
| M5 / M6 | ⬜ |
...
- 主人多一組工具 `remember_global_rule` / `list_global_rules` / `forget_global_rule`，
  寫的是**所有伺服器都適用**的規則（一般人只寫得到自己伺服器那一份，而且看不到主人工具的存在）
...
```powershell
# 455 項驗收測試：搜尋、群組、匹配、通知判定、簡繁折疊、縮寫、輪詢成本、儲存後端與 DI 註冊、.env 路徑、合併與復原、結束追蹤、AI 聊天切段與每週額度、LLM 工具訂閱、可馴服的提示詞、認人與主人授權、面板與 AI 共用站牌解析、路線號碼與轉乘查詢、模型思考開關、SQLite 持久化
