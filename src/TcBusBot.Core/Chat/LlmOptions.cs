...
public sealed class LlmOptions
{
    ...
    /// <summary>
    /// 讓模型看到 **Discord 暱稱**（伺服器暱稱／顯示名稱），而不是 `@帳號`。
    /// 環境變數：`LLM_SHOW_NICKNAMES`（預設 `true`）。
    ///
    /// 為什麼預設要開：頻道上大家互相稱呼的是暱稱（「小明」），不是帳號（`@wuxiaohan0922`）。
    /// 模型要聽得懂「小明剛剛說的」「叫小明來」就必須看到暱稱 ——
    /// 這在**判斷「這句話是不是在對我說話」**與**判斷話題**時特別重要，
    /// 因為那兩段提示詞原本只寫帳號，模型根本認不出誰是誰。
    ///
    /// 關掉的好處：同一個人在不同伺服器有一致的身分（暱稱每個伺服器都不一樣），
    /// 而且不會出現兩個不同的人剛好取同一個暱稱。
    ...
    /// <summary>
    /// 讓模型知道**它在這個伺服器被叫什麼**（Bot 被改暱稱時很常見）。
    /// 環境變數：`LLM_SHOW_NICKNAMES`（跟上面同一個開關）。
    ///
    /// 例：伺服器把 Bot 改叫「貓貓」，那在提示詞裡就是「大家叫你『貓貓』」——
    /// 否則它只認得自己的帳號，有人喊「貓貓」它不會覺得是在叫它。
    /// </summary>
    ...
    ///
    /// ⚠️ 重點是「**只有使用者指定才會用**」：
    ///   * 只有**對 Bot 說話**的訊息（@ 它、回覆它）才可能附圖
    ///   * 偷聽到的訊息、別人聊天裡的照片**一律不送**（成本 ＋ 隱私）
    ///   * 回覆某張圖的訊息時，被回覆那一則的圖片也算（「這張圖是什麼？」很自然）
    ///
    /// 成本：每一張圖最多 **1024 tokens**（官方換算：縮到約 1300×1300 的總像素），
    /// 所以預設一次最多 <see cref="VisionMaxImages"/> 張。
    /// 關掉時，有圖片的那一則會留一句「（有 N 張圖片，主機沒有開啟圖片理解）」
    /// —— 讓模型知道自己看不到，而不是憑空編造。
    /// </summary>
    public bool Vision { get; set; } = true;

    /// <summary>
    /// 一次最多附幾張圖片（`LLM_VISION_MAX_IMAGES`，預設 2）。
    ///
    /// 為什麼要上限：一張圖最多 1024 tokens，貼十張就是 10k tokens；
    /// 而且「一次看兩張」通常已經足夠回答「這張圖是什麼」。
    /// </summary>
    public int VisionMaxImages { get; set; } = 2;
