...
└─ src/TcBusBot.Cli/             ← 離線開發工具（`tcbus`）
    ...
    └─ SelfTest.cs                      ★ 684 項離線驗收測試（搜尋、匹配、儲存與 DI、AI 聊天與工具、可馴服的提示詞、偷聽模式…）
```
...
```powershell
dotnet run --project src\TcBusBot.Cli -- selftest              # 684 項驗收
...
使用者回報「LLM 似乎沒有接上」時，最難的其實不是修，而是**分辨是哪一種沒接上**：

| 症狀 | 真正的原因 |
| --- | --- |
| 它完全沒回我 | 私訊？（`LLM_ALLOW_DM=false` 時設計上就不回）／沒 @ 到它／頻道權限 |
| 在群組裡不回 | 偷聽判斷成 SKIP／STOP（設計如此） |
| 回一句「額度用完了」 | 每週 token 上限到了 |
| 回「❌ 呼叫失敗：…」 | 金鑰 401、餘額 402、模型名稱 404、逾時… |
| 連「正在輸入…」都沒有 | 訊息根本沒進來（Message Content 意圖沒開） |

以前只能靠 console log（在 Render 上，要登進去翻），而 `/ai status` 只寫「已回幾則／失敗幾次」——
**沒有「最後一次失敗的訊息是什麼」**，所以使用者無從判斷。

#### 20.24.1 作法

| 元件 | 內容 |
| --- | --- |
| `LlmDiagnostics`（Core） | 留最近 **20** 筆呼叫紀錄（成功：模型／in-out／耗時；失敗：**錯誤訊息原文**），`Describe()` 給 `/ai status` 用 |
| `RoutingLlmClient` 記帳 | 它是**所有**呼叫的必經之路（聊天、工具、短判斷），在這裡記就一定記得到；例外原樣往上丟（呼叫端靠它決定「先不出聲」） |
| `/ai test` | 用一次真的呼叫確認：成功回內容＋模型＋用量＋往返時間；失敗把 **API 的原話**貼出來，並依錯誤碼給方向（401 → 金鑰、402 → 餘額、404 → 模型名稱、429 → 限流、逾時） |
| `/ai status` 的「最近幾次呼叫」 | 最後一次（成功或失敗）＋最後一次失敗的完整訊息＋最近 20 筆裡失敗幾筆 |
| 私訊的說明 | 收到私訊且 `LLM_ALLOW_DM=false` 時，**回一句說明**（每人 6 小時一次）：「我預設不在私訊回話，請到頻道 @ 我」 |

| 決定 | 為什麼 |
| --- | --- |
| 診斷紀錄由路由層統一記（不是各偵測器自己記） | 「有沒有接上」必須涵蓋**每一條**呼叫路徑；分散著記一定會漏 |
| `/ai test` 只開放管理員／已授權的人 | 它會花掉真的額度（一次幾十 tokens），不能變成任何人燒錢的按鈕 |
| `/ai test` 的呼叫**也記進每週額度** | 不然「測試」就是繞過上限的後門 |
| 失敗時貼 API 的原話，而不是只說「失敗」 | 401／402／404／429 各自對應完全不同的處置；翻譯成中文反而讓使用者查不到 |
| 私訊只回一次說明（每人 6 小時） | 完全不回會被當成壞掉，一直回又變成洗頻 |

#### 20.24.2 驗收

`tcbus selftest` 新增 **8 項**：

| 檢查 | 驗什麼 |
| --- | --- |
| 沒紀錄時 `Describe()` 講得清楚（不是空白） | `/ai status` 對「還沒有人問過它」也要有話說 |
| 成功的呼叫記得下模型與用量 | 診斷的基本資料 |
| 失敗的呼叫記得下**錯誤訊息**、`Describe()` 含 `401` | 「接不上」的證據 |
| 之後若成功，會同時標出「最後一次」與「最後一次失敗」 | 兩者都看得到 |
| 筆數與失敗數算得出來；紀錄有上限（只留 20 筆） | 不會無限長大 |
| 經過路由層的呼叫都會被記下來；沒有診斷物件時也不會壞 | 接線與可選性 |
...
### 20.26 每週額度的「重置」按鈕（`/ai status`，管理員）

每週 token 額度是**硬的**（用完就直接拒絕回話，見 §20.13），而它以前只能靠
「改環境變數 ＋ 重啟服務」或「等到 UTC 週一 00:00」恢復。實際會遇到的狀況是
**額度剛好在最需要它的時候用完**（大家在問路），那時候去改 Render 的環境變數
再重啟，是為了讓 Bot 今天能回話而做的最大手術。

所以 `/ai status` 底部多了一顆「♻️ 重置本週額度」，只有 `LLM_ADMIN_IDS` 看得到。

| 決定 | 為什麼 |
| --- | --- |
| 掛在 `/ai status` 上（不是新指令） | 「額度剩多少」跟「把額度歸零」是同一件事的兩面，看得到的地方就按得到。而且斜線指令**模型碰不到**（見 §20.19 的說明），不會被提示注入叫去按 |
| **一定要按兩次** | 第一次只顯示「目前用了多少、重置會／不會做什麼」，第二次才真的做。這顆按鈕動的是**錢**，不該一顆按鈕擦掉 |
| 按鈕只認**開啟它的那個人**（`custom_id` 尾端帶 ID），而且按下時**再檢查一次**是不是管理員 | 「看不到就按不到」不算安全機制。兩道都要有：非管理員一定被擋；兩個管理員之間也不能互相代按（否則「按兩次」的確認就形同虛設） |
| 授權判斷寫成**純函式** `QuotaResetPolicy.DenyReason` | 安全性判斷最怕「只寫在文件上、程式碼裡沒擋」。抽出來之後離線驗證可以拿各種身分直接餵它（非管理員／別的管理員／沒設名單） |
| 一定寫**主人操作紀錄**（`/ai audit`） | 不然「額度怎麼突然變多」會變成查不出來的事 —— 這種紀錄的價值就在出事的那一天 |
| 額度用完的訊息**只對管理員**多一句提示 | 一般使用者按不到，跟他講只是多一行字；管理員則正好在那個當下需要知道怎麼救 |

`WeeklyTokenBudget.Reset` 本來就在（測試在用它），這次把它接上使用者介面，並且
補上「**重置要寫進儲存區**」的驗收 —— 少了這一步，重啟之後舊用量會回來，等於沒重置。

驗收：

| 在哪 | 驗什麼 |
| --- | --- |
| `tcbus selftest`（新增 **6 項**） | 重置前**先確認真的會被擋**（不然這組驗證沒意義）、重置把用量／次數／被擋次數／來源統計全部歸零、重置後額度立刻恢復、**重置要寫進儲存區**、額度用完的訊息會告訴管理員去按按鈕（一般使用者不會多那句） |
| `--dryrun`（新增一節「額度重置按鈕檢查」） | 把三排按鈕真的畫出來（含跨伺服器授權通知那兩顆）讓接線檢查對照；餵四種身分給授權純函式；解析 `custom_id` 的嚴格性；用 **IL 掃描**確認「按第一次不會重置」「取消不會重置」「確定才呼叫 `WeeklyTokenBudget.Reset` 且留下 `AuditAdmin`」 |

##### 順手修掉：模組清單有三份，其中一份漏了東西

做這顆按鈕時發現 `PersonaGrantModule`（§20.23 的授權按鈕）**沒有註冊進 DI**，
也不在「按鈕 ↔ 處理函式」接線檢查的掃描範圍裡。它剛好還能動，因為
Discord.Net 會用 `ActivatorUtilities` 現場建實例 —— 但那就代表：

* `ValidateOnBuild` 擋不到它的建構子依賴寫錯（少一個註冊 → 執行時才炸）
* 它的按鈕完全沒被「按了有沒有反應」檢查涵蓋

所以把「有哪些模組」收成**一份** `BotModules.All`，DI 註冊、啟動註冊、
離線驗證三邊都從同一份長出來（`--dryrun` 現在會說「7 個指令模組」而不是 5 個）。
接線檢查也順便升級成掃**全部**模組，於是每個處理函式都對得到一顆真的畫出來的按鈕。

再補一道防線：`--dryrun` 會**掃組件裡所有 `InteractionModuleBase` 的子類別**，
確認每一個都在 `BotModules` 清單裡（漏掉＝線上不存在那組指令，而且不會報錯）。
這道防線自己就抓到了兩件事 —— 第一次寫的時候 7 個只掃到 3 個，因為大部分模組是
繼承 `BusModuleBase`（為了 `UpdateAsync`），只看**直接**基底類別會漏掉它們；
所以它改成往上走完整條繼承鏈。
